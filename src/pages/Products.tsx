...
interface VariantForm {
  ...
  color: string; // Dynamic Color Field
  sku: string;
  unit_price: string;
...
const EMPTY_FORM: ProductForm = {
  ...
  sku: "",
  variants: [
    { id: "v1", variation_value: "", color: "", sku: "", unit_price: "", stock_quantity: "" },
  ],
};
...
export default function Products() {
  ...
  const [dbCategories, setDbCategories] = useState<{ name: string }[]>([]); // Dynamic Categories
  ...
  function openEdit(p: Product) {
    ...
    setForm({
      ...
      has_variants: true,
      variation_type: p.variation_type || "Size & Color",
      size: "",
      unit_price: "",
      stock_quantity: "",
      sku: "",
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (isAddingNewCat) {
      if (!newCatName.trim()) {
        setFormError("New category name is required");
        return;
      }
    ...
    const validVariants = form.variants.filter(
      (v) => v.variation_value.trim() && v.unit_price.trim() && v.stock_quantity.trim(),
    );
    ...
    if (!profile?.id) {
      setFormError("You must be signed in to save products.");
      return;
    }
                          ...
                          {hasVariants ? (
                            <div className="flex flex-wrap items-center gap-1">
                              <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-medium bg-brand-50 dark:bg-brand-900/20 text-brand-600 dark:text-brand-400">
                                <Layers size={12} />
                                {p.variation_type || "Size & Color"} ({p.variants!.length})
                              </span>
                            </div>
                          ) : (
                              ...
                              <table className="w-full text-xs">
                                <thead>
                                  <tr className="border-b border-slate-200 dark:border-slate-700">
                                    <th className="text-left py-2 px-3 font-medium text-slate-500 uppercase">
                                      Size
                                    </th>
                                    <th className="text-left py-2 px-3 font-medium text-slate-500 uppercase">
                                      Color
                                    ...
                                    <th className="text-right py-2 px-3 font-medium text-slate-500 uppercase">
                                      Unit Price
                                    </th>
                                    <th className="text-right py-2 px-3 font-medium text-slate-500 uppercase">
                                      Stock
                                  ...
                                  {p.variants!.map((v: ProductVariant) => (
                                    ...
                                    >
                                      ...
                                      <td className="py-2 px-3 text-right text-slate-600 dark:text-slate-300">
                                        {formatCurrency(Number(v.unit_price) || 0)}
                                      </td>
      ...
      >
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              value={isAddingNewCat ? "NEW" : form.category}
              onChange={(e) => {
            ...
            {isAddingNewCat && (
              <input
                type="text"
                placeholder="Type new category name (e.g. Duvet Cover)"
                value={newCatName}
                onChange={(e) => setNewCatName(e.target.value)}
                className="input-field animate-scale-in"
                required
              />
            )}
          </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. Satin Stripe Duvet Set"
              required
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {form.variants.map((v) => (
                <div
                  key={v.id}
                  className="grid grid-cols-[1fr_1fr_90px_80px_32px] gap-2 items-center p-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/30 animate-scale-in"
                >
                  <input
                    type="text"
            ...
            <div className="flex flex-wrap gap-1.5">
              {COMMON_SIZES.filter((s) => !form.variants.some((v) => v.variation_value === s)).map(
                (s) => (
                  <button
                    key={s}
                    type="button"
